<?php namespace App\Http\Librerias;
...
class CaracteresEspeciales{
    ...
    public static function getFieldsIndex($table, $table_t = null, $field_index)
    {
        ...
        if ($table_t != null)
        {
            $estructura_tabla_translation = \DB::select('SHOW FULL COLUMNS FROM ' . $table_t);
        }
                        ...
                        if (strpos($field->Field, 'imagen') === false)
                        {
                            if (strpos($field->Type, 'blob') === false) {
                                if (strpos($field->Type, 'varchar') === false) {
                                    if (strpos($field->Type, 'tinyint') === false && strpos($field->Type, 'double') === false && strpos($field->Type, 'text') === false) {
    ...
    public static function getFieldsCreate($table, $translatable = null, $fields_excluded, $validators){
        $fields = [];
        $estructura_tabla = \DB::select('SHOW FULL COLUMNS FROM ' . $table);
        if ($translatable != null)
        {
            $estructura_tabla_translation = $translatable;
        }
            ...
            if ($field->Key != 'PRI') {
                if ($field->Field != 'created_at' && $field->Field != 'updated_at') {
                    if (in_array($field->Field, $fields_excluded)) {
                        ...
                        if($translatable!=null)
                        {
                            if (in_array($field->Field, $translatable))
                            {
                            ...
                            if (strpos($field->Type, 'varchar') === false)
                            {
                                if (strpos($field->Type, 'double') === false)
                                {
                                    if (strpos($field->Type, 'decimal') === false)
                                    {
                                        if (strpos($field->Type, 'int') === false)
                                        {
                                            if (strpos($field->Type, 'text') === false && strpos($field->Type, 'mediumtext') === false)
                                            {
                                        ...
                                        else
                                        {
                                            ...
                                            elseif ($field->Field=='id_color')
                                            {
                                                $elemento->mostrar = 'relacion';
                                            }
                                            elseif ($field->Field=='id_medida')
                                            {
                                                $elemento->mostrar = 'relacion';
                                            }
                                            else
                                            {
                                                $elemento->mostrar = 'updown';
                                            }
                                        }
                                    }
                                    else
                                    {
                                        $elemento->mostrar = 'precio';
                                    }
                                }
                                else
                                {
    ...
    public static function getFieldsUpdate($table, $translatable = null, $fields_excluded, $validators){
        $fields = [];
        $estructura_tabla = \DB::select('SHOW FULL COLUMNS FROM ' . $table);
        if ($translatable != null) $estructura_tabla_translation = $translatable;
        foreach ($estructura_tabla as $field) {
            if ($field->Key != 'PRI') {
                if ($field->Field != 'created_at' && $field->Field != 'updated_at') {
                    if (in_array($field->Field, $fields_excluded)) {
